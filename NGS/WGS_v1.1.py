...
###############################################################################Variable table
#Directory list
dir_list = ['01.Trimmomatic','02.BAM','03.GATK','04.ReadCount','05.VCF','06.VEP']
#Main path
main_path  = "/data/WES/"
#Tools path
tool_path = "/home/oem/Program/"
...
def BWA(file):
    ...
    for sample_dir in sample_dirs:
        ...
        for id_left_paired in glob.glob("*_1.paired.fq"):
                ...
                if id_sam not in os.listdir(sample_path+sample_dir+"/"+dir_list[1]):
                    cmd = f"{tool_path}bwa-0.7.17/bwa mem -t {thread} -k 45 /data/hg38/Homo_sapiens_assembly38.fasta {id_left_paired} {id_right_paired} > {sample_path+sample_dir+'/'+dir_list[1]+'/'+id_sam}"
                    print(cmd)
                    os.system(cmd)
    ...
    for sample_dir in sample_dirs:
        ...
        for id_sam in glob.glob("*.sam"):
            ...
            if id_sorted_bam not in os.listdir(os.curdir):
                cmd = f"{tool_path}samtools-1.12/samtools view -bS {id_sam} | {tool_path}samtools-1.12/samtools sort -@ {thread} -O bam -o {id_sorted_bam}"
                print(cmd)
                os.system(cmd)
    ...
    for sample_dir in sample_dirs:
        ...
        for id_bam in glob.glob("*sorted.bam"):
            ...
            if RG_bam not in os.listdir(os.curdir):
                cmd = f"{tool_path}gatk-4.1.8.1/gatk --java-options '-Xmx{ram} -XX:+UseParallelGC -XX:ParallelGCThreads={thread}' AddOrReplaceReadGroups --SORT_ORDER coordinate --INPUT {id_bam} --OUTPUT {RG_bam} --RGID SH --RGLB WES --RGPL illumina --RGPU OV --RGSM {id_bam.split('-WES')[0]} 2> {RG_log}"
                print(cmd)
                os.system(cmd)
    ...
    for sample_dir in sample_dirs:
        ...
        for RG_bam in glob.glob("*.RG.bam"):
            ...
            if marked_bam not in os.listdir(sample_path+sample_dir+"/"+dir_list[2]):
                cmd = f"{tool_path}gatk-4.1.8.1/gatk --java-options '-Xmx{ram} -XX:+UseParallelGC -XX:ParallelGCThreads={thread}' MarkDuplicates -I {RG_bam} -O ../{dir_list[2]}/{marked_bam} -M ../{dir_list[2]}/{marked_mtr} --VALIDATION_STRINGENCY SILENT --ASSUME_SORTED true --REMOVE_DUPLICATES 2> ../{dir_list[2]}/{marked_log}"
                print(cmd)
                os.system(cmd)
    ...
    for sample_dir in sample_dirs:
        ...
        for marked_bam in glob.glob("*RG.dedup.bam"):
            ...
            if bqsr_log not in os.listdir(os.curdir):
                cmd1 = f"{tool_path}gatk-4.1.8.1/gatk --java-options '-Xmx{ram} -XX:+UseParallelGC -XX:ParallelGCThreads={thread}' BaseRecalibrator -I {marked_bam} -R /data/hg38/Homo_sapiens_assembly38.fasta --known-sites /data/hg38/1000G_phase1.snps.high_confidence.hg38.vcf.gz --known-sites /data/hg38/Mills_and_1000G_gold_standard.indels.hg38.vcf.gz -O {bqsr_table} 2> {bqsr_log}"
                print(cmd1)
                os.system(cmd1)
			    
                bqsr_bam = marked_bam.replace("RG.dedup.bam","RG.dedup.recal.bam")
                applybqsr_log = bqsr_log.replace("bqsr.log","applybqsr.log")
                cmd2 = f"{tool_path}gatk-4.1.8.1/gatk --java-options '-Xmx{ram} -XX:+UseParallelGC -XX:ParallelGCThreads={thread}' ApplyBQSR -I {marked_bam} -O {bqsr_bam} --bqsr-recal-file {bqsr_table} 2> {applybqsr_log}"
                print(cmd2)
                os.system(cmd2)
    ...
    for sample_dir in sample_dirs:
        if "-T" in sample_dir:
            ...
            for recal_bam in glob.glob("*.recal.bam"):
                ...
                os.chdir(sample_path+sample_dir+"/"+dir_list[4])
                if vc_log not in os.listdir(os.curdir):
                    cmd1 = f"{tool_path}gatk-4.1.8.1/gatk --java-options '-Xmx{ram} -XX:+UseParallelGC -XX:ParallelGCThreads={thread}' Mutect2 -R /data/hg38/Homo_sapiens_assembly38.fasta -I ../{dir_list[2]}/{recal_bam} -tumor {tumor_name} -I {sample_path}{norm_name}/{dir_list[2]}/{norm_recal_bam} -normal {norm_name} -pon /data/hg38/somatic-hg38_1000g_pon.hg38.vcf.gz --germline-resource /data/hg38/somatic-hg38_af-only-gnomad.hg38.vcf.gz --ignore-itr-artifacts --af-of-alleles-not-in-resource 0.0000025 --disable-read-filter MateOnSameContigOrNoMappedMateReadFilter --bam-output {tumor_name}.mutect2.bam -O {tumor_name}.mutect2.unfiltered.vcf 2> {vc_log}"
                    print(cmd1)
                    os.system(cmd1)
                    ...
                    cmd2 = f"{tool_path}gatk-4.1.8.1/gatk --java-options '-Xmx{ram} -XX:+UseParallelGC -XX:ParallelGCThreads={thread}' FilterMutectCalls --max-alt-allele-count 5 -R /data/hg38/Homo_sapiens_assembly38.fasta -V {uf_file} -O {f_file} 2> {fmc_log}"
                    print(cmd2)
                    os.system(cmd2)

                    #"PASS" extraction
                    pass_file = f_file.replace(".vcf",".passed.vcf")
                    cmd3 = f"/home/oem/Program/bcftools-1.10.2/bin/bcftools view -f 'PASS' {f_file} > {pass_file}"
                    print(cmd3)
                    os.system(cmd3)
...
def VEP(file):
    ...
    for sample_dir in sample_dirs:
        ...
        for passed_vcf in glob.glob("*.passed.vcf"):
            ...
            if vep_vcf not in os.listdir(sample_path+sample_dir+"/"+dir_list[5]):
                cmd = f"{tool_path}ensembl-vep/vep --force_overwrite --cache --format vcf --vcf -i {passed_vcf} -o ../{dir_list[5]}/{vep_vcf} --fork 32 --symbol --terms SO --tsl --hgvs --pick --refseq REFSEQ_MATCH --use_given_ref --plugin Downstream --plugin Wildtype --plugin Frameshift --fasta /data/hg38/Homo_sapiens_assembly38.fasta --dir_plugins /home/oem/.vep/Plugins"
                print(cmd)
                os.system(cmd)
