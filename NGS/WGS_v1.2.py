    ...
    for sample_dir in sample_dirs:
        ...
        for passed_vcf in glob.glob("*.passed.vcf"):
            ...
            if vep_vcf not in os.listdir(sample_path+sample_dir+"/"+dir_list[4]):
                cmd = f"{tool_path}ensembl-vep/vep --force_overwrite --cache --format vcf --vcf -i {passed_vcf} -o ../{dir_list[4]}/{vep_vcf} --fork 32 --symbol --terms SO --tsl --hgvs --pick --refseq REFSEQ_MATCH --use_given_ref --plugin Downstream --plugin Wildtype --plugin Frameshift --fasta /data/hg38/Homo_sapiens_assembly38.fasta --dir_plugins /home/oem/.vep/Plugins"
                print(cmd)
                os.system(cmd)
