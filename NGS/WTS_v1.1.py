...
##########################################################################Variable table
#Directoty list
dir_list = ['01.Trimmomatic','02.Align','03.Cufflinks','04.HLAtyping','05.ReadCount']
#Main path
main_path  = "/data/WTS/"
#Tools path
...
def Trimmomatic(file):
    ...
    if not id_log in os.listdir(os.curdir):
        cmd = f'java -jar {tool_path}trimmomatic-0.39/trimmomatic-0.39.jar PE -threads {thread} -phred33 {srd_path}{sample_name} {srd_path}{id_right} {id_left_output_paired} {id_left_output_unpaired} {id_right_output_paired} {id_right_output_unpaired} AVGQUAL:20 LEADING:3 TRAILING:3 MINLEN:50 SLIDINGWINDOW:4:15 2> {id_log}'
        print(cmd)
        os.system(cmd)
...
def Align(file):
	...
	if not "align_summary.txt" in os.listdir(sample_path+sample_name_spl+"/"+dir_list[1]):
		cmd = f'{tool_path}tophat-2.1.1.Linux_x86_64/tophat --output-dir ../02.Align --num-threads {thread} --mate-std-dev 56 --library-type fr-unstranded /data/hg38/Homo_sapiens/NCBI/GRCh38/Sequence/Bowtie2Index/genome {R1_file} {R2_file}'
		print(cmd)
		os.system(cmd)
